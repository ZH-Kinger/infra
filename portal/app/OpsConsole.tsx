    ...
    <main className="console-shell">
      <aside className="sidebar">
        <div className="brand">
          ...
          <div><strong>DATASET OPS</strong><small>CONTROL PLANE</small></div>
        </div>
        <nav aria-label="主导航">
          {nav.map((item) => (
        ...
        <div className="environment-card">
          ...
          <div><small>ENVIRONMENT</small><strong>CN-HANGZHOU · DEMO</strong></div>
        </div>
      ...
      <section className="workspace">
        <header className="topbar">
          <div><span className="eyebrow">LAKEFS · OSS · CPFS · PAI</span><h1>{title}</h1></div>
          <div className="top-actions"><span className="health"><i />SYSTEM HEALTHY</span><button onClick={() => setActive("adopt")}>＋ 纳管数据集</button></div>
        </header>
...
function Overview({ operations, onNavigate }: { operations: Operation[]; onNavigate: (key: NavKey) => void }) {
  return <div className="page-grid">
    <section className="hero-panel">
      <div><span className="eyebrow">TRAINING DATA FABRIC</span><h2>把每一次训练，锁定在<br />可验证的数据版本上。</h2><p>从存量数据纳管到 GPU 消费，所有变化通过 Commit、审批与最小权限角色完成。</p></div>
      <div className="fabric-map"><div className="map-node">OSS<small>持久层</small></div><b>→</b><div className="map-node accent">lakeFS<small>版本真相</small></div><b>→</b><div className="map-node">CPFS<small>热副本</small></div><b>→</b><div className="map-node">PAI<small>训练消费</small></div></div>
    </section>
    <div className="metric-row">
      <Metric label="MANAGED DATA" value="136.4" unit="TB" note="3 个正式数据集" />
      <Metric label="CPFS UTILIZATION" value="68.2" unit="%" note="2.47 PB / 3.60 PB" tone="amber" />
      <Metric label="ACTIVE RUNTIMES" value="27" unit="" note="DSW 11 · DLC 16" />
      <Metric label="POLICY VIOLATIONS" value="0" unit="" note="最近 24 小时" tone="green" />
    </div>
...
function DatasetTable({ compact = false }: { compact?: boolean }) {
  return <div className="data-table"><div className="table-row head"><span>DATASET</span><span>COMMIT</span><span>SIZE</span><span>VERSIONS</span><span>STATUS</span>{!compact && <span>CPFS HOT</span>}</div>{datasets.map((item) => <div className="table-row" key={item.name}><span><b className="dataset-mark" /> <strong>{item.name}</strong></span><span className="mono">{item.commit}</span><span>{item.size}</span><span>{item.versions}</span><span><i className={`tag ${item.state.toLowerCase()}`}>{item.state}</i></span>{!compact && <span><b className="bar"><i style={{ width: `${item.hot}%` }} /></b>{item.hot}%</span>}</div>)}</div>;
}

function DatasetInventory() {
  return <div className="page-grid"><section className="panel span-12"><div className="inventory-title"><div><span className="eyebrow">CATALOGUE</span><h2>受治理的数据资产</h2></div><div className="filter-chip">3 DATASETS · 33 VERSIONS</div></div><DatasetTable /></section><section className="panel span-7"><PanelHead title="版本完整性" /><div className="integrity-list"><p><span>Commit 锁定率</span><strong>100%</strong></p><p><span>Manifest SHA-256 覆盖</span><strong>100%</strong></p><p><span>PAI Version 注册一致性</span><strong>97.4%</strong></p></div></section><section className="panel span-5"><PanelHead title="存储分布" /><div className="donut"><div><strong>136</strong><small>TB TOTAL</small></div><p><span><i className="blue-dot" /> CPFS HOT 68%</span><span><i className="dim-dot" /> OSS ONLY 32%</span></p></div></section></div>;
}

function Toggle({ execute, setExecute }: { execute: boolean; setExecute: (value: boolean) => void }) {
  return <label className="execute-toggle"><input type="checkbox" checked={execute} onChange={(e) => setExecute(e.target.checked)} /><span /><div><strong>{execute ? "EXECUTE" : "PLAN ONLY"}</strong><small>{execute ? "将进入审批环境" : "不会修改云资源"}</small></div></label>;
}
