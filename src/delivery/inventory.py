...
为什么看板读快照、不直接调云 API
──────────────────────────────
① 一个能读 RAM/IAM 的看板进程，本身就是值得攻击的东西。展示走快照，就不需要为了
   「看一眼」而在每个请求里拿着云凭证。
   **注意这句话今天只对了一半**：面板进程现在确实握着开通身份的 AK（开通、离职停号
   都在面板里调云，见 provision.executor_from_env、offboard.py）。所以真正的护栏
   不再是「没有凭证」，而是那把 AK 在云上的权限被收窄过、外加受保护账号的 Deny
   （deploy/panel/cloud-policies/）。别再照着「面板没有凭证」这个前提做设计。
