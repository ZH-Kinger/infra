...
# ---------------------------------------------------------------------------
# 时机约束：**这个模块必须在目录还空着的时候 apply。**
#
# CPFS 里已经有文件的目录**不能**注册成 Fileset——只能建在新的/空的路径上，
# 没有「就地转换」这回事。
#
# 连锁后果比它看起来大：数据流动的第一条前提是必须挂 Fileset（`FsetId` 必填），
# 所以一旦某个目录先有了数据、再想给它配数据流动，就只能：
#
#   1. 新建一个 Fileset 路径；
#   2. 把数据搬过去（同文件系统内的 rename 是元数据操作，秒级；但**跨 Fileset
#      边界未必**，规划迁移前要先实测，别假设一定是零拷贝）；
#   3. 再建 DataFlow。
#
# 对存量 CPFS 环境接入本项目，这一步是绕不过去的，要写进迁移计划而不是
# 等到 apply 报错才发现。
# ---------------------------------------------------------------------------
