...
class WorkflowTriggerIsolationTests(unittest.TestCase):
    ...
    def test_datalake_itest_uses_essd_and_reachable_image_sources(self):
        ...
        self.assertIn("spark.sql.catalog.datalake.type: rest", spark)
        self.assertIn("spark.sql.catalog.datalake.uri: http://polaris:8181/api/catalog", spark)
        self.assertIn("iceberg-aws-bundle:1.10.2", spark)
    ...
    def test_datalake_itest_models_shared_s3_and_file_namespace(self):
        ...
        self.assertIn("--name=$(POD_NAME)", juicefs)
        self.assertNotIn("--name=$(ETCD_NAME)", juicefs)
        self.assertNotIn("--name=$(HOSTNAME)", juicefs)
        old_etcd_shell_entrypoint = (
            'command: ["/bin/sh", "-ec"]\n          args:\n            - |\n              exec etcd'
        )
        self.assertNotIn(old_etcd_shell_entrypoint, juicefs)
        ...
        self.assertIn(".status.currentRevision", deploy)
        self.assertIn(".status.updateRevision", deploy)
